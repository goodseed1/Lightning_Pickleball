...
Lightning Pickleball 앱에서 **모든 NTRP 용어와 코드를 LPR로 완전히 마이그레이션**합니다.
...
7. `src/screens/CreateEventForm.tsx` - 매치 생성 시 LPR/NTRP
...
### NTRP (1.0-5.0+) → LPR (1-10) 변환

현재 온보딩에서 사용하는 NTRP 값을 LPR로 변환:
...
async function main() {
  ...
  console.log('🚀 Starting NTRP to LPR migration...');
  await migrateUsers();
...
| #   | 테스트 케이스           | 예상 결과                   | 검증 방법      |
| --- | ----------------------- | --------------------------- | -------------- |
| 1.1 | LPR 레벨 선택 화면 표시 | 1-10 레벨 + 티어 표시       | UI 확인        |
| 1.2 | 자가 진단 시작          | LPR 진단 화면 표시          | UI 확인        |
| 1.3 | 진단 완료               | LPR 레벨 결과 표시          | UI 확인        |
...
| 2.2 | 복식 매치 생성   | 파트너 LPR 평균 계산 정확 | 계산 검증      |
| 2.3 | LPR 범위 표시    | "LPR 5-7" 형식            | UI 확인        |
...
| #   | 테스트 케이스    | 예상 결과          | 검증 방법    |
| --- | ---------------- | ------------------ | ------------ |
| 3.1 | 적합 LPR 신청    | 신청 성공          | 앱 동작 확인 |
| 3.2 | 부적합 LPR 신청  | 에러 메시지 + 거부 | UI 확인      |
| 3.3 | 에러 메시지 내용 | "LPR 불일치" 표시  | 번역 확인    |

### Test Suite 4: 프로필 화면

| #   | 테스트 케이스      | 예상 결과                     | 검증 방법      |
| --- | ------------------ | ----------------------------- | -------------- |
| 4.1 | 내 프로필 LPR 표시 | "LPR 7" 형식                  | UI 확인        |
| 4.2 | 타 유저 프로필 LPR | "LPR 5" 형식                  | UI 확인        |
| 4.3 | LPR 수정           | Firestore `ltrLevel` 업데이트 | Firestore 확인 |

### Test Suite 5: 발견(Discovery) 화면

| #   | 테스트 케이스      | 예상 결과           | 검증 방법 |
| --- | ------------------ | ------------------- | --------- |
| 5.1 | LPR 필터링         | 범위 내 매치만 표시 | UI 확인   |
| 5.2 | 매치 카드 LPR 표시 | "LPR 4-6" 형식      | UI 확인   |
...
| 6.2 | 기존 유저 프로필 (ntrpLevel)  | LPR로 변환 표시 | UI 확인      |

### Test Suite 7: AI 챗봇

| #   | 테스트 케이스       | 예상 결과          | 검증 방법   |
| --- | ------------------- | ------------------ | ----------- |
| 7.1 | LPR 관련 질문       | LPR 스케일로 응답  | 챗봇 테스트 |
| 7.2 | 파트너 갭 규칙 질문 | "±2 LPR 이내" 응답 | 챗봇 테스트 |

### Test Suite 8: 번역 검증 (10개 언어)

| 언어            | 확인 항목                        |
| --------------- | -------------------------------- |
| 영어 (en)       | "LPR Level", "Your LPR: {{ltr}}" |
| 한국어 (ko)     | "LPR 레벨", "내 LPR: {{ltr}}"    |
| 스페인어 (es)   | "Nivel LPR"                      |
| 중국어 (zh)     | "LPR 等级"                       |
| 일본어 (ja)     | "LPRレベル"                      |
| 프랑스어 (fr)   | "Niveau LPR"                     |
| 독일어 (de)     | "LPR-Stufe"                      |
| 이탈리아어 (it) | "Livello LPR"                    |
| 포르투갈어 (pt) | "Nível LPR"                      |
| 러시아어 (ru)   | "Уровень LPR"                    |
