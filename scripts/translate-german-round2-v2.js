...
// Apply translations only where de[key] === en[key] (untranslated)
function applyTranslations(enObj, deObj, translations, path = '') {
  let count = 0;
  ...
  for (const key in translations) {
    ...
    if (typeof translations[key] === 'object' && !Array.isArray(translations[key])) {
      // Ensure nested object exists in de
      if (!deObj[key]) {
        deObj[key] = {};
      }
      count += applyTranslations(enObj[key] || {}, deObj[key], translations[key], currentPath);
    } else {
      // Only apply if currently untranslated (de === en)
      if (enObj[key] && deObj[key] === enObj[key]) {
        deObj[key] = translations[key];
        count++;
        console.log(`   ✓ ${currentPath}: "${enObj[key]}" → "${translations[key]}"`);
      }
    }
  }
...
const translated = applyTranslations(en, de, germanTranslations);

// Statistics after
console.log('\n📊 AFTER TRANSLATION:');
const untranslatedAfter = countUntranslated(en, de);
