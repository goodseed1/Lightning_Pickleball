...
// Translation map for all 1135 untranslated keys
const translations = {
  // ADMIN SECTION (33 keys)
...
function applyTranslations(obj, path = '') {
  for (const key in obj) {
    ...
    if (typeof obj[key] === 'object' && obj[key] !== null && !Array.isArray(obj[key])) {
      applyTranslations(obj[key], fullPath);
    } else if (translations[fullPath]) {
      obj[key] = translations[fullPath];
    }
  }
}
...
console.log('✅ Applied ' + Object.keys(translations).length + ' translations to zh.json');
