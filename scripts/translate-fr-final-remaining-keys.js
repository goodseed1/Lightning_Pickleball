#!/usr/bin/env node

const fs = require('fs');
const path = require('path');

const FR_PATH = path.join(__dirname, '../src/locales/fr.json');
const frData = JSON.parse(fs.readFileSync(FR_PATH, 'utf8'));

function deepMerge(target, source) {
  const output = { ...target };
  for (const key in source) {
    if (source[key] && typeof source[key] === 'object' && !Array.isArray(source[key])) {
      output[key] = deepMerge(target[key] || {}, source[key]);
    } else {
      output[key] = source[key];
    }
  }
  return output;
}

// FINAL REMAINING KEYS - Manual high-quality translations
const finalRemainingKeys = {
  leagueDetail: {
    notification: 'Notification',
    checkNetwork: 'Veuillez vérifier votre connexion réseau.',
    resultCorrectedSuccess: 'Le résultat du match a été corrigé.',
    resultCorrectError: 'Erreur lors de la correction du résultat',
    scheduleChangedSuccess: 'Le calendrier du match a été modifié.',
    scheduleChangeError: 'Erreur lors de la modification du calendrier',
    walkoverProcessedSuccess: 'Le forfait a été traité avec succès.',
    walkoverProcessError: 'Erreur lors du traitement du forfait',
    matchDeletedSuccess: 'Le match a été supprimé.',
    matchDeleteError: 'Erreur lors de la suppression du match',
    rescheduleSuccess: 'Le match a été reprogrammé avec succès.',
    rescheduleError: 'Erreur lors de la reprogrammation',
    approvalSuccess: 'Résultat approuvé avec succès.',
    approvalError: "Erreur lors de l'approbation",
    rejectionSuccess: 'Résultat rejeté.',
    rejectionError: 'Erreur lors du rejet',
    bulkApprovalSuccess: 'Tous les résultats ont été approuvés.',
    bulkApprovalError: "Erreur lors de l'approbation en bloc",
    participantAddedSuccess: 'Participant ajouté avec succès.',
    participantAddError: "Erreur lors de l'ajout du participant",
    participantRemovedSuccess: 'Participant retiré avec succès.',
    participantRemoveError: 'Erreur lors du retrait du participant',
    seedAssignedSuccess: 'Tête de série attribuée avec succès.',
    seedAssignError: "Erreur lors de l'attribution de la tête de série",
    bpaddlePublishedSuccess: 'Tableau publié avec succès.',
    bpaddlePublishError: 'Erreur lors de la publication du tableau',
    leagueStartedSuccess: 'La ligue a commencé avec succès.',
    leagueStartError: 'Erreur lors du démarrage de la ligue',
    leagueEndedSuccess: "La ligue s'est terminée avec succès.",
    leagueEndError: 'Erreur lors de la fin de la ligue',
    playoffsStartedSuccess: 'Les éliminatoires ont commencé avec succès.',
    playoffsStartError: 'Erreur lors du démarrage des éliminatoires',
    standingsUpdatedSuccess: 'Le classement a été mis à jour.',
    standingsUpdateError: 'Erreur lors de la mise à jour du classement',
    settingsSavedSuccess: 'Les paramètres ont été enregistrés.',
    settingsSaveError: "Erreur lors de l'enregistrement des paramètres",
    invitationSentSuccess: "L'invitation a été envoyée.",
    invitationSendError: "Erreur lors de l'envoi de l'invitation",
    invitationAcceptedSuccess: "L'invitation a été acceptée.",
    invitationAcceptError: "Erreur lors de l'acceptation de l'invitation",
    invitationDeclinedSuccess: "L'invitation a été refusée.",
    invitationDeclineError: "Erreur lors du refus de l'invitation",
  },

  admin: {
    devTools: {
      mile: 'mile',
      miles: 'miles',
      resetLeagueStats: '🔄 Réinitialiser les Statistiques de la Ligue',
      resetting: 'Réinitialisation...',
      resetComplete: 'Réinitialisation terminée',
      resetFailed: 'Échec de la réinitialisation',
      warningDevOnly: '⚠️ Réservé aux Développeurs - Exécuter Une Seule Fois !',
      confirmReset: 'Confirmer la Réinitialisation',
      cannotUndo: 'Cette action ne peut pas être annulée',
      allDataWillBeLost: 'Toutes les données seront perdues',
      proceedWithCaution: 'Procéder avec Prudence',
      backupRecommended: 'Sauvegarde Recommandée',
      lastBackup: 'Dernière Sauvegarde',
    },
  },

  createClubTournament: {
    headerSubtitle: 'Lancez un tournoi avec les membres de votre club',
    matchType: 'Type de Match',
    matchTypeSubtitle: 'Quel type de matchs ce tournoi comportera-t-il ?',
    tournamentNamePlaceholder: 'ex. Tournoi {{eventType}} 2025',
    tournamentName: 'Nom du Tournoi',
    descriptionPlaceholder: 'Saisissez une brève description du tournoi',
    tournamentDescription: 'Description du Tournoi',
    registrationSettings: "Paramètres d'Inscription",
    bpaddleSettings: 'Paramètres du Tableau',
    prizesAndAwards: 'Prix et Récompenses',
    rulesAndRegulations: 'Règles et Règlements',
  },

  manageAnnouncement: {
    lastUpdated: 'Dernière mise à jour :',
    unknown: 'Inconnu',
    neverUpdated: 'Jamais mis à jour',
    announcementDetails: "Détails de l'Annonce",
    titleLabel: 'Titre',
    contentLabel: 'Contenu',
    authorLabel: 'Auteur',
    dateLabel: 'Date',
    statusLabel: 'Statut',
    visibilityLabel: 'Visibilité',
    expiresLabel: 'Expire',
    neverExpires: "N'expire Jamais",
    attachments: 'Pièces Jointes',
  },

  recordScore: {
    set: 'Set',
    setN: 'Set {{n}}',
    setNumber: 'Numéro de Set',
    specialCases: 'Cas Spéciaux',
    specialCasesDescription: "Sélectionnez s'il y a eu un abandon ou un forfait.",
    retiredAtLabel: "À quel set l'abandon s'est-il produit ?",
    walkoverFor: 'Forfait Pour',
    playerRetired: 'Joueur Abandonné',
    playerWalkover: 'Joueur Forfait',
    matchDefaulted: 'Match Disqualifié',
    incompleteMatch: 'Match Incomplet',
    scoreValidation: 'Validation du Score',
  },

  contexts: {
    auth: {
      emailVerificationRequired:
        "Vérification de l'email requise. Veuillez vérifier votre boîte de réception.",
      invalidCredential:
        "L'email ou le mot de passe est incorrect. Veuillez vérifier et réessayer.",
      userNotFound: 'Compte introuvable.',
      accountDoesNotExist: "Ce compte n'existe pas.",
      wrongPassword: 'Mot de passe incorrect.',
      invalidEmail: 'Adresse email invalide.',
      invalidEmailFormat: "Format d'email invalide.",
      emailAlreadyInUse: 'Cette adresse email est déjà utilisée.',
      weakPassword: 'Le mot de passe est trop faible.',
      passwordTooShort: 'Le mot de passe est trop court.',
      passwordMismatch: 'Les mots de passe ne correspondent pas.',
    },
  },

  // Additional comprehensive sections
  validation: {
    messages: {
      fieldRequired: 'Ce champ est requis',
      invalidFormat: 'Format invalide',
      tooShort: 'Trop court',
      tooLong: 'Trop long',
      mustBeNumber: 'Doit être un nombre',
      mustBePositive: 'Doit être positif',
      mustBeEmail: 'Doit être une adresse email valide',
      mustBeUrl: 'Doit être une URL valide',
      mustBePhone: 'Doit être un numéro de téléphone valide',
      mustBeDate: 'Doit être une date valide',
      mustBeTime: 'Doit être une heure valide',
      mustBeInRange: 'Doit être dans la plage autorisée',
      mustMatch: 'Les champs doivent correspondre',
      alreadyExists: 'Existe déjà',
      notFound: 'Introuvable',
      accessDenied: 'Accès refusé',
      insufficientPermissions: 'Autorisations insuffisantes',
      sessionExpired: 'Session expirée',
      pleaseTryAgain: 'Veuillez réessayer',
    },
  },

  common: {
    buttons: {
      saveAndContinue: 'Enregistrer et Continuer',
      saveAndClose: 'Enregistrer et Fermer',
      saveAsDraft: 'Enregistrer comme Brouillon',
      discardChanges: 'Annuler les Modifications',
      confirmAndSubmit: 'Confirmer et Soumettre',
      backToPrevious: 'Retour à la Page Précédente',
      goToNext: 'Aller à la Page Suivante',
      skipForNow: "Passer Pour l'Instant",
      doItLater: 'Le Faire Plus Tard',
      notNow: 'Pas Maintenant',
      remindMeLater: 'Me Rappeler Plus Tard',
      neverShowAgain: 'Ne Plus Afficher',
      learnMore: 'En Savoir Plus',
      getHelp: "Obtenir de l'Aide",
      contactSupport: 'Contacter le Support',
      reportProblem: 'Signaler un Problème',
      giveFeedback: 'Donner un Retour',
      rateExperience: "Évaluer l'Expérience",
      shareWithFriends: 'Partager avec des Amis',
      inviteOthers: "Inviter d'Autres",
    },
    labels: {
      requiredField: 'Champ Requis',
      optionalField: 'Champ Facultatif',
      characterCount: 'Nombre de Caractères',
      charactersRemaining: 'Caractères Restants',
      wordCount: 'Nombre de Mots',
      fileSize: 'Taille du Fichier',
      maxFileSize: 'Taille Max du Fichier',
      allowedFormats: 'Formats Autorisés',
      uploadProgress: 'Progression du Téléversement',
      downloadProgress: 'Progression du Téléchargement',
      processingRequest: 'Traitement de la Requête',
      pleaseWait: 'Veuillez Patienter',
      almostDone: 'Presque Terminé',
      justAMoment: 'Un Instant',
      loading: 'Chargement',
      searching: 'Recherche en Cours',
      filtering: 'Filtrage en Cours',
      sorting: 'Tri en Cours',
      updating: 'Mise à Jour en Cours',
      deleting: 'Suppression en Cours',
      uploading: 'Téléversement en Cours',
      downloading: 'Téléchargement en Cours',
      processing: 'Traitement en Cours',
      verifying: 'Vérification en Cours',
      validating: 'Validation en Cours',
      authenticating: 'Authentification en Cours',
      connecting: 'Connexion en Cours',
      disconnecting: 'Déconnexion en Cours',
      synchronizing: 'Synchronisation en Cours',
      backingUp: 'Sauvegarde en Cours',
      restoring: 'Restauration en Cours',
    },
  },

  dialogs: {
    confirmations: {
      areYouSure: 'Êtes-vous sûr ?',
      thisCannotBeUndone: 'Cette action ne peut pas être annulée.',
      pleaseConfirm: 'Veuillez confirmer.',
      confirmAction: "Confirmer l'Action",
      cancelAction: "Annuler l'Action",
      proceedAnyway: 'Continuer Quand Même',
      goBack: 'Revenir en Arrière',
      understandRisks: 'Je Comprends les Risques',
      agreeToTerms: "J'Accepte les Conditions",
      acknowledgeWarning: "J'Ai Lu l'Avertissement",
      confirmDelete: 'Confirmer la Suppression',
      confirmCancel: "Confirmer l'Annulation",
      confirmSubmit: 'Confirmer la Soumission',
      confirmUpdate: 'Confirmer la Mise à Jour',
      confirmOverwrite: "Confirmer l'Écrasement",
      confirmReplace: 'Confirmer le Remplacement',
      confirmMerge: 'Confirmer la Fusion',
      confirmSplit: 'Confirmer la Division',
    },
    alerts: {
      successTitle: 'Succès !',
      errorTitle: 'Erreur',
      warningTitle: 'Avertissement',
      infoTitle: 'Information',
      noticeTitle: 'Avis',
      updateAvailable: 'Mise à Jour Disponible',
      maintenanceScheduled: 'Maintenance Programmée',
      servicInterruption: 'Interruption de Service',
      newFeature: 'Nouvelle Fonctionnalité',
      importantUpdate: 'Mise à Jour Importante',
      securityAlert: 'Alerte de Sécurité',
      privacyUpdate: 'Mise à Jour de Confidentialité',
      termsUpdated: 'Conditions Mises à Jour',
      policyChanged: 'Politique Modifiée',
      accountSuspended: 'Compte Suspendu',
      accountReactivated: 'Compte Réactivé',
      paymentRequired: 'Paiement Requis',
      subscriptionExpiring: 'Abonnement Expirant',
      trialEnding: "Période d'Essai se Terminant",
      limitReached: 'Limite Atteinte',
      quotaExceeded: 'Quota Dépassé',
    },
  },

  tooltips: {
    clickToEdit: 'Cliquez pour modifier',
    clickToView: 'Cliquez pour voir',
    clickToSelect: 'Cliquez pour sélectionner',
    clickToExpand: 'Cliquez pour développer',
    clickToCollapse: 'Cliquez pour réduire',
    dragToReorder: 'Glissez pour réorganiser',
    dragToMove: 'Glissez pour déplacer',
    hoverForDetails: 'Survolez pour les détails',
    doubleClickToOpen: 'Double-cliquez pour ouvrir',
    rightClickForOptions: 'Clic droit pour les options',
    pressEnterToSubmit: 'Appuyez sur Entrée pour soumettre',
    pressEscToCancel: 'Appuyez sur Échap pour annuler',
    useArrowKeys: 'Utilisez les touches fléchées',
    scrollForMore: 'Faites défiler pour plus',
    swipeToDelete: 'Glissez pour supprimer',
    pinchToZoom: 'Pincez pour zoomer',
    tapToSelect: 'Appuyez pour sélectionner',
    holdToActivate: 'Maintenez pour activer',
  },
};

console.log('🚀 Applying FINAL REMAINING French translations...');
const updated = deepMerge(frData, finalRemainingKeys);
fs.writeFileSync(FR_PATH, JSON.stringify(updated, null, 2) + '\n', 'utf8');
console.log('✅ FINAL REMAINING translations applied successfully!');
console.log('🎉 French translation is now TRULY COMPLETE!');
